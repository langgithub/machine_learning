...
"""
import time
from nlp import helper
import argparse
import numpy as np
import pandas as pd
import tensorflow as tf
from nlp.BILSTM_CRF import BILSTM_CRF
...
with tf.Session(config=config) as sess:
    with tf.device(gpu_config):
        ...
        with tf.variable_scope("model", reuse=None, initializer=initializer):
            model = BILSTM_CRF(num_chars=num_chars, num_classes=num_classes, num_steps=num_steps, num_epochs=num_epochs,
